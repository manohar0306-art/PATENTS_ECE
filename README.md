# <p align="center">🚀 Patents Portfolio</p>

<p align="center">
  <img src="https://img.shields.io/badge/Innovator-Manohar-0077b6?style=for-the-badge&logo=github" />
  <img src="https://img.shields.io/badge/Patents--Filed-03-gold?style=for-the-badge&logo=google-scholar" />
  <img src="https://img.shields.io/badge/Research--Status-Active-brightgreen?style=for-the-badge&logo=micro-soft" />
</p>

---

## 💎 The Patent Portfolio
Below are my officially published patents, representing solutions for critical real-world problems in the fields of IoT, Health-Tech, and Industrial Safety.

| Sl. No | Project Dashboard | Core Technology | Primary Impact |
| :---: | :--- | :--- | :--- |
| 01 | 🚑 **[Smart Ambulance System](./01-Smart_Ambulance_System)** | GPS + nRF24L01 + DSP | Medical Transit & Traffic Control |
| 02 | 🔥 **[Wireless Mesh Fire Detection](./02-Wireless_Mesh_Fire_Detection)** | LoRa Mesh + Solar P2P | Forest Fire Prevention |
| 03 | ⛑️ **[Mining Safety Helmet](./03-Smart_Mining_Safety_Helmet)** | UWB + LoRa + Gas Sensors | Underground Miner Protection |

---

## 🧠 My Research Philosophy
> *"Research is not just a process; it's a passion to solve what hasn't been solved yet."*

I have a profound passion for research and the pursuit of original solutions. To me, engineering is the art of integrating hardware and software to solve the world's most complex challenges. This isn't just a hobby—it is my **primary mission**.

My goal is to identify systemic gaps in society and create simplified, high-tech solutions. By protecting these innovations through the legal patent process, I ensure that technology serves humanity securely and effectively.

## 🔮 Future Vision & Roadmap
Innovation is a persistent journey. I am committed to expanding this portfolio further:
* **Continuous Research:** Currently exploring **Edge AI and TinyML** to make sensors smarter.
* **Problem Solving:** Identifying gaps in Smart City infrastructure to provide sustainable tech solutions.
* **Upcoming Patents:** Already working on the documentation for **2 more innovative concepts** that I plan to file as patents soon.

---

## 🛠️ Technical Skill Spectrum
<p align="left">
  <img src="https://img.shields.io/badge/Microcontrollers-%20ESP32%20|%20Arduino-blue?style=flat-square" />
  <img src="https://img.shields.io/badge/Wireless-LoRa%20|%20UWB%20|%20nRF24-orange?style=flat-square" />
  <img src="https://img.shields.io/badge/Design-PCB%20|%20Fusion%20360%20|%20KiCad-red?style=flat-square" />
</p>

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&height=100&section=footer"/>

</div>
